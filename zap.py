...
def process(musecubefits, outcubefits='DATACUBE_FINAL_ZAP.fits', clean=True, zlevel='median', 
            q=0, cfilter=100, pevals=[], nevals=[], optimize=False, silent=False, extSVD=''):
    """
    ...
    zobj._run(clean=clean, zlevel=zlevel, q=q, cfilter=cfilter, 
              pevals=pevals, nevals=nevals, optimize=optimize, silent=silent, extSVD=extSVD)
    
    zobj.mergefits(outcubefits)


def interactive(musecubefits, clean=True, zlevel='median', q=0, cfilter=100, pevals=[], 
...
def SVDoutput(musecubefits, svdfn='ZAP_SVD.fits', clean=True, zlevel='median', q=0, 
    ...
    zobj._continuumfilter(cfilter=cfilter)
...
class zclass:
    ...
    def _run(self, clean=True, zlevel='median', q=0, cfilter=100, pevals=[], nevals=[], 
             optimize=False, silent=False, extSVD=''):
...
##output a stack of spectra and arrays with the coordinates
    ...
    def _zlevel(self, calctype='median', q=0):
        ...
        if calctype != 'none':
            ...
            if calctype == 'median':
                ...
                self.zlsky = np.median(zlstack, axis=1)
            ...
            #Do an iterative sigma clip
                ...
                print 'Unrecognized zlevel procedure'
                self.zlsky=np.zeros(self.stack.shape[0])
    ...
    def _continuumfilter(self, cfilter=100, silent=False):
        ...
        for i in range(len(medianranges)): 
            p = multiprocessing.Process(target=_icontinuumfilter, args=(i,
                                                                         self.stack,
                                                                         self._cfilter,
                                                                         medianranges[i],
                                                                         return_dict))
        ...
        self.contarray=np.concatenate(return_dict, axis=1)
    ...
    def optimize(self):
        ...
        for i in range(nseg):
            ...
            #look for crossing points. When they get within 3 sigma of mean in settled region.
    ...
    def writeSVD(self, svdfn='ZAP_SVD.fits'):
        ...
        #append the zlevel subtraction if it exists
        #if self.run_zlevel != 'none':
        hdu.append(pyfits.PrimaryHDU(self.zlsky))
        #else:
        #    hdu.append(pyfits.PrimaryHDU())
        
        #append the eigenspectra
...
##### Continuum Filtering #####
def _icontinuumfilter(i, stack, cfilter, sprange, return_dict):  #for distributing data to the pool
    """
...
def _isreconstruct(zobj, seg, i):  
    ...
    """
    iespeceval=zobj.especeval[seg]
    eigenspectra, evals = iespeceval
    ieigenspectra = eigenspectra[:,i]
    ievals = evals[i,:]
    reconpiece = ieigenspectra[:,np.newaxis] * ievals[np.newaxis,:] # broadcast evals on evects and sum
    scube = np.zeros([evals.shape[0],zobj.cube.shape[1],zobj.cube.shape[2]])*np.nan
    scube[:,zobj.y,zobj.x] = reconpiece

    return scube

def _irreconstruct(zobj, seg, rng):  
    """
    Reconstruct the residuals from a given set of eigenspectra and eigenvalues.
    ...
    iespeceval=zobj.especeval[seg]
    eigenspectra, evals = iespeceval
    ieigenspectra = eigenspectra[:,rng[0]:rng[1]]
    ievals = evals[rng[0]:rng[1],:]
    nrows=(ievals.shape)[1]
    reconpiece=np.zeros([(eigenspectra.shape)[0],nrows]) # make container
    for i in np.arange(nrows): # this loop is FASTER than a fully vectorized one-liner command
        evalvect=(ievals[:,i]) # choose single eval set
        reconpiece[:,i]=np.sum(ieigenspectra * evalvect,axis=1) # broadcast evals on evects and sum
...
def plotvarcurve(zobj, i=0):
    ...
    plt.plot([zobj.nevals[i],zobj.nevals[i]],[min(varcurve),max(varcurve)])

    ax= fig.add_subplot(3,1,2)
    plt.plot(np.arange(deriv.size)+1,deriv)
    plt.plot([1,noptpix-1],[mn1,mn1])
    plt.plot([1,noptpix-1],[mn1-2*std1,mn1-2*std1])
    plt.plot([zobj.nevals[i]+1,zobj.nevals[i]+1],[min(deriv),max(deriv)])
    ...
    plt.plot([2,noptpix-2],[mn2+2*std2,mn2+2*std2])
    plt.plot([zobj.nevals[i]+2,zobj.nevals[i]+2],[min(deriv2),max(deriv2)])
    plt.suptitle(i)
...
def _isigclip(i, stack, pranges, return_dict):
    ...
    ## Clean up the nan value spaxels
def nanclean(cube, rejectratio=0.25, boxsz=1, silent=False):
    """
    Detects NaN values in cube and removes them by replacing them with an 
    interpolation of the nearest neighbors in the data cube. The positions in the cube 
    are retained in nancube for later remasking.
